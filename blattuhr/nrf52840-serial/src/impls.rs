...
use postcard_rpc::{
    ...
    server::{WireRx, WireRxErrorKind, WireTx, WireTxErrorKind}, standard_icd::LoggingTopic, Topic,
};
use serde::Serialize;

pub struct EioTxInner<Tx: Write> {
    pub t: Tx,
    pub tx_buf: &'static mut [u8],
    pub log_seq: u16,
}

pub struct EioTx<R, Tx>
where
    R: RawMutex + 'static,
    Tx: Write + 'static,
{
    pub t: &'static Mutex<R, EioTxInner<Tx>>,
}

impl<R, Tx> Clone for EioTx<R, Tx>
where
    R: RawMutex + 'static,
    Tx: Write + 'static,
{
...
fn body_to_flavor<'a, T: Serialize + ?Sized>(msg: &T, flava: Cobs<Slice<'a>>) -> Result<&'a [u8], WireTxErrorKind> {
...
impl<R, Tx> WireTx for EioTx<R, Tx>
where
    R: RawMutex + 'static,
    Tx: Write + 'static,
{
    ...
    async fn send<T: Serialize + ?Sized>(
        ...
        let EioTxInner { t, tx_buf, .. } = guard.deref_mut();
    ...
    async fn send_raw(&self, buf: &[u8]) -> Result<(), Self::Error> {
        ...
        flavor.try_extend(buf)
            .map_err(|_| WireTxErrorKind::Other)?;
        let used = flavor.finalize()
            .map_err(|_| WireTxErrorKind::Other)?;
    ...
    async fn send_log_str(&self, kkind: VarKeyKind, s: &str) -> Result<(), Self::Error> {
        let mut guard = self.t.lock().await;
        let EioTxInner { t, tx_buf, log_seq } = guard.deref_mut();
...
impl<R: Read> WireRx for EioRx<R> {
